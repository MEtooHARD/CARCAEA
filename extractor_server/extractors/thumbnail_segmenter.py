...
class ThumbnailSegmenter:
    ...
    def find_thumbnail(
        ...
        Args:
            ...
            is_preprocessed: 若為 True，表示輸入的 chroma 已經預先降採樣過了（例如來自 CENS），直接使用而不再降採樣
        ...
        if is_preprocessed:
            # 輸入已經預先降採樣（但不一定是 1 Hz）
            # CENS 輸出的頻率是 ~11 Hz（hop_length=512, sr=22050, downsample_factor=4）
            # 需要再降採樣到 1 Hz
            chroma_cens = chroma
            cens_frame_rate = frames_per_sec / 4  # CENS 的降採樣因子通常是 4
            cens_downsample_to_1hz = int(np.round(cens_frame_rate))  # ~11
            chroma_downsampled = self._downsample_chroma(chroma_cens, cens_downsample_to_1hz)
            print(f"[ThumbnailSegmenter] ℹ️  使用預處理的 CENS chroma：{cens_frame_rate:.1f} Hz → 1 Hz (降採樣因子: {cens_downsample_to_1hz})")
        else:
            # 原始邏輯：計算需要的降採樣因子
        ...
        if music_envelope is not None:
            if is_preprocessed:
                # 當輸入 chroma 已預處理時，能量包絡線也應該已經調整
                # 直接使用，但確保長度與 chroma_downsampled 匹配
                if len(music_envelope) != chroma_downsampled.shape[1]:
                    # 如果長度不匹配，進行簡單的線性插值調整
                    # 或者簡單地截斷/擴展
                    target_len = chroma_downsampled.shape[1]
                    if len(music_envelope) > target_len:
                        env_normalized = music_envelope[:target_len]
                    else:
                        # 簡單的重複填充
                        repeat_factor = target_len // len(music_envelope) + 1
                        env_normalized = np.tile(music_envelope, repeat_factor)[:target_len]
                else:
                    env_normalized = music_envelope
                
                # 正規化
                env_min = np.min(env_normalized)
                env_max = np.max(env_normalized)
                if env_max > env_min:
                    env_normalized = (env_normalized - env_min) / (env_max - env_min)
                else:
                    env_normalized = np.ones_like(env_normalized)
            else:
                # 原始邏輯：計算降採樣因子下的能量包絡線
                frames_per_sec = self.sr / self.hop_length
                downsample_factor = int(frames_per_sec)
            ...
            print(f"  • 能量範圍 (正規化後): [{np.min(env_normalized):.4f}, {np.max(env_normalized):.4f}]")
            print(f"  • 能量向量長度: {len(env_normalized)}")
        else:
            env_normalized = None
