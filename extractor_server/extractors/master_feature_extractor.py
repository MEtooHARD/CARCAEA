...
from .cens_extractor import CENSExtractor
from .chroma_pitch_extractor import ChromaPitchExtractor
...
class MasterFeatureExtractor:
    ...
    def __init__(self, sr: Optional[int] = None, hop_length: int = 512):
        ...
        self.cens_extractor: Optional[CENSExtractor] = None
        self.chroma_pitch_extractor: Optional[ChromaPitchExtractor] = None
    ...
    def _init_extractors(self, sr: int) -> None:
        ...
        if self.tempo_extractor is None:
            ...
            self.cens_extractor = CENSExtractor(
                sr=sr,
                hop_length=self.hop_length,
                log_compression_factor=100.0,
                smoothing_window_sec=1.0,
                downsample_factor=4
            )
            self.chroma_pitch_extractor = ChromaPitchExtractor(sr=sr, hop_length=self.hop_length)
    ...
    async def extract_medical_grade_features(
        ...
        # ============================================
        # Phase 2.5: 基於 CENS SSM 的縮圖分割
        # ============================================
        print(f"[Phase 2.5] Finding representative thumbnail via CENS-SSM segmentation...")
        # 使用 CENS（高質量色度特徵）替代傳統 chroma_stft
        # CENS 管道包括：CQT + 對數壓縮 + 歸一化 + 時間平滑 + 降採樣
        chroma = self.cens_extractor.extract(audio_mono)
        print(f"[Phase 2.5] ✓ CENS chroma matrix extracted: {chroma.shape}")
        ...
        thumb_start_time, thumb_end_time, thumb_start_frame, thumb_end_frame = \
            self.thumbnail_segmenter.find_thumbnail(
                ...
                is_preprocessed=True  # CENS 已預先降採樣
            )
        ...
        # ===== NEW: 為完整曲子提取 Chroma 特徵 =====
        print(f"[Phase 3] Extracting Chroma features for full song...")
        chroma_full_results = self.chroma_pitch_extractor.extract_from_audio(
            audio_mono,
            sr=sr,
            hop_length=self.hop_length
        )
        
        dominant_pitch_full_frames = chroma_full_results["dominant_pitch_class"]
        chroma_flux_full_frames = chroma_full_results["chroma_flux"]
        times_chroma_full = chroma_full_results["times"]
        
        # 重採樣全曲 Chroma 特徵到 4Hz
        resampler_full_chroma = Resampler(full_duration)
        dominant_pitch_full_4hz = resampler_full_chroma.resample_envelope(
            dominant_pitch_full_frames,
            times_chroma_full,
            allow_nan=False
        )
        chroma_flux_full_4hz = resampler_full_chroma.resample_envelope(
            chroma_flux_full_frames,
            times_chroma_full,
            allow_nan=False
        )
        
        print(f"[Phase 3] ✓ Full song Chroma extracted: {len(dominant_pitch_full_4hz)} samples @ 4Hz")
        ...
        # ===== NEW: 用 Chroma Dominant Pitch Class 替代 pYIN F0 (解決 EDM 無法偵測) =====
        print(f"[Phase 3] Extracting Chroma-based dominant pitch class and Chroma flux...")
        # 提取縮圖對應的音頻片段（樣本）
        thumb_start_sample = int(thumb_start_time * sr)
        thumb_end_sample = int(thumb_end_time * sr)
        audio_thumbnail = audio_mono[thumb_start_sample:thumb_end_sample]

        # 使用 ChromaPitchExtractor 提取主導音級與色度通量
        chroma_results = self.chroma_pitch_extractor.extract_from_audio(
            audio_thumbnail, 
            sr=sr, 
            hop_length=self.hop_length
        )
        ...
        # 提取 chroma 特徵
        dominant_pitch_class_frames = chroma_results["dominant_pitch_class"]  # Shape: (num_frames,)
        chroma_flux_frames = chroma_results["chroma_flux"]  # Shape: (num_frames,)
        times_chroma = chroma_results["times"]  # Shape: (num_frames,)

        # 重採樣到 4Hz（與其他包絡線對齐）
        resampler_chroma = Resampler(thumbnail_duration_actual)
        
        # 直接重採樣主導音級（1-12 或 0）
        dominant_pitch_4hz = resampler_chroma.resample_envelope(
            dominant_pitch_class_frames,
            times_chroma,
            allow_nan=False
        )
        
        # 重採樣色度通量
        chroma_flux_4hz = resampler_chroma.resample_envelope(
            chroma_flux_frames,
            times_chroma,
            allow_nan=False
        )
        ...
        # 計算統計指標
        chroma_flux_mean = float(np.mean(chroma_flux_4hz))
        chroma_flux_std = float(np.std(chroma_flux_4hz))
        chroma_flux_var = float(np.var(chroma_flux_4hz))  # NEW: 計算變異數
        dominant_pitch_nonzero = dominant_pitch_4hz[dominant_pitch_4hz > 0]
        dominant_pitch_mean = float(np.mean(dominant_pitch_nonzero)) if len(dominant_pitch_nonzero) > 0 else 0.0
        
        print(f"[Phase 3] ✓ Chroma features extracted @ 4Hz")
        print(
            f"  - Dominant pitch class: mean={dominant_pitch_mean:.1f} (1-12 scale)")
        print(
            f"  - Chroma flux: mean={chroma_flux_mean:.4f}, std={chroma_flux_std:.4f}, var={chroma_flux_var:.4f}")
        ...
        hrv_features = self.aggregator.aggregate_for_hrv_prediction(
            ...
            thumbnail_f0_envelope_4hz=dominant_pitch_4hz,
            thumbnail_loudness_envelope_4hz=loudness_resampled,
            thumbnail_pyin_f0_envelope_4hz=f0_resampled,
            
            # NEW: Chroma-based F0 特徵（用於 EDM/重金屬無法偵測的情況）
            thumbnail_dominant_pitch_4hz=dominant_pitch_4hz,
            thumbnail_chroma_flux_4hz=chroma_flux_4hz,
            chroma_flux_mean=chroma_flux_mean,
            chroma_flux_std=chroma_flux_std,
            chroma_flux_var=chroma_flux_var,  # NEW: 變異數
            dominant_pitch_mean=dominant_pitch_mean,
            
            # NEW: 全曲 4Hz 重採樣包絡線（命名與縮圖驗證陣列一致）
            full_pyin_f0_envelope_4hz=f0_full_4hz,
            full_dominant_pitch_4hz=dominant_pitch_full_4hz,
            full_chroma_flux_4hz=chroma_flux_full_4hz,
            full_music_envelope_4hz=music_full_4hz,
            full_loudness_envelope_4hz=loudness_full_4hz,
        )
        ...
        return {
            ...
            "full_features": hrv_features["full_features"],  # 使用 aggregator 返回的 full_features
